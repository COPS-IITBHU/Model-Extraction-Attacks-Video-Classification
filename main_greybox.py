...
from vidmodex.train import train_Wdata
from vidmodex.utils.dataloader import videosDataset

def greybox_main(data_config):
    ...
    generator.load_state_dict(torch.load(data_config["ucf_gan_weights"])['model_state_dict'][0])
    ...
    best_acc = 0
    acc_list = []
    writer = SummaryWriter(f"runs/GB")
    step = 0
    for epoch in range(1, number_epochs + 1):
        ...
        loss_S, loss_G = train_Wdata(config_args, teacher=teacher, student=student, generator=generator, dataloader=train_loader,
