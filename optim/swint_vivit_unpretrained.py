...
def train(config_args, teacher, student, generator, device, optimizer, epoch):
    ...
    for i in range(config_args.epoch_itrs):
        ...
        torch.save(generator, "swinT_viviT_weights_2/gen_model_%d.pt" % i)
        torch.save(student, "swinT_viviT_weights_2/student_model_%d.pt" % i)
        torch.save(teacher,  "swinT_viviT_weights_2/teacher_model_%d.pt" % i)
...
def main():
    ...
    best_acc = 0
    acc_list = []
    writer = SummaryWriter(f"runs/SwinT/BB_untrained")
    step = 0
    for epoch in range(1, number_epochs + 1):
        ...
        if acc > best_acc:
            ...
            name = 'swinT'
            torch.save(student.state_dict(
            ), f"check_untrain/student_kinetics400-{name}.pth")
            torch.save(generator.state_dict(
            ), f"check_untrain/student_kinetics400-generator.pth")
        # vp.add_scalar('Acc', epoch, acc)
        if config_args.store_checkpoints:
            torch.save(student.state_dict(), config_args.log_dir +
                       f"/check_untrain/student.pth")
            torch.save(generator.state_dict(), config_args.log_dir +
                        f"/check_untrain/generator.pth")
